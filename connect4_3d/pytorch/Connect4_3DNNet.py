...
class Connect4_3DNNet(nn.Module):
    def __init__(self, game, args):
        # Game params.
        self.game = game
        self.board_x, self.board_y, self.board_z = self.game.getBoardSize()
        ...
        super(Connect4_3DNNet, self).__init__()
        
        self.conv = nn.Conv3d(in_channels=self.input_channels,
    ...
    def forward(self, s):
        s = s.view(-1, self.input_channels, self.board_x, self.board_y, self.board_z) # Add channel dimension.
        if self.args.dual:
            s = self.conv(s)
            s = self.body(s)
            s = self.head(s)
...
class Block(nn.Module):
    ...
    def get_left(self):
        ...
        for repeat in range(self.args.block_repeats):
            ...
            layers.append(nn.Conv3d(num_channels,
                                    self.args.num_channels,
                                    self.args.kernel_size,
                                    stride=1,
                                    padding=self.args.kernel_size//2))
            if self.args.useBatchNorm:
                layers.append(nn.BatchNorm3d(self.args.num_channels))
...
class Pi(nn.Module):
    def __init__(self, game, args):
        ...
        if self.args.action2D:
            self.board_x, self.board_y, self.board_z = self.game.getBoardSize()
            self.input_num = self.args.num_channels * self.board_z
        else:
            self.input_num = self.args.dense_layers[-1]
        super(Pi, self).__init__()
        if self.args.action2D:
            self.layers = nn.ModuleList([Reshape(-1, self.input_num, self.board_x, self.board_y),
                                         nn.Conv2d(self.input_num, 
                                                   1,
                                                   self.args.kernel_size,
                                                   stride=1,
                                                   padding=self.args.kernel_size//2),
            ...
            self.layers = nn.ModuleList([nn.Linear(self.input_num, self.action_size)])
...
class V(nn.Module):
    def __init__(self, game, args):
        ...
        if self.args.action2D:
            self.board_x, self.board_y, self.board_z = self.game.getBoardSize()
            self.input_num = self.args.num_channels * self.board_x * self.board_y * self.board_z
        else:
            self.input_num = self.args.dense_layers[-1]
